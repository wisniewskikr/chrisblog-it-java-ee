...
##### Goal
The goal of this project is to present how to implement **EJB** with scope **Stateless** in **REST API** application using **Java** programming language and **Java EE 9** framework. Application is deployed on a server **payara micro** in version 6.
...
* **Stateless**: EJB with this scope is created by application server and stored in the pool. Application server provides it to application if it's needed. We shouldn't store any state is such beans because they are commonly used by all users
