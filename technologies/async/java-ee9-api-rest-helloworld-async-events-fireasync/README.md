...
##### Goal
The goal of this project is to present how to create **asynchronous event** using **Event.fireAsync()** method in a **REST API Java** application using **Java EE 9** framework. Application is deployed on **payara micro** in version 6.

**Event** means that there is one or many **observes** who wait for event. If event happens then observes are informed and also do some action. By default events are synchronous - main thread is blocked until observes finish their actions. But calling Event.fireAsync() method and using in event listeners @ObservesAsync makes events asynchronous - main thread is not blocked any more.
