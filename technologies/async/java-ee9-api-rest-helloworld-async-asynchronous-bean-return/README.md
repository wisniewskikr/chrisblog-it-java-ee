...
1. In a http browser (e.g. Chrome) visit `http://localhost:8080/rest/api/hello`
   * Expected JSON **{"message": "Hello World!"}** in a browser **after 5 seconds**
...
##### Goal
The goal of this project is to present how to use **asynchronous bean** with **return** method in **REST API Java** application using **Java EE 9** framework. 

Application is deployed on **payara micro** in version 6.

**Ansychronous bean** is a bean which is started in separate thread. In this example it returns message. It means that messages in a browser and in a console are displayed after 5 secodns delay. Advantage of this approach is that request-response thread is free when second thread is working.
